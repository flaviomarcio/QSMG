#pragma once

#include "./p_qsmg_guardian_thread.h"
#include <QLocalServer>
#include <QLocalSocket>
#include <QTimer>

namespace QSMG {

class Server:public GuardianThread
{
    Q_OBJECT
public:
    QLocalServer* server=nullptr;
    QString port;
    QHash<QLocalSocket*, QByteArray> clientsidentifier;

    //!
    //! \brief Server
    //! \param port
    //!
    explicit Server(const QString&port): GuardianThread()
    {
        this->port=port;
        this->moveToThread(this);
    }

    //!
    //! \brief run
    //!
    void run()override
    {
        this->server = new QLocalServer(this);
        QObject::connect(this->server, &QLocalServer::newConnection,this, &Server::onServerNewConnection);

        //start server
        QTimer::singleShot(10, this, &Server::onServerStart);

        this->exec();

        QHashIterator<QLocalSocket*, QByteArray> i(this->clientsidentifier);
        while(i.hasNext()){
            i.next();
            auto client=i.key();
            client->close();
            client->deleteLater();
        }

        if(server!=nullptr){
            QObject::disconnect(this->server, &QLocalServer::newConnection,this, &Server::onServerNewConnection);
            server->disconnect();
            delete server;
            server=nullptr;
        }
    }

public slots:

    //!
    //! \brief onServerStart
    //!
    void onServerStart()
    {
        if(server==nullptr)
            return;

        if(server->isListening())
            return;

        if (!server->listen(port)) {
            qWarning()<<tr("Server: Cannot bind on port %1: %2").arg(port, server->errorString());
            this->quit();
            return;
        }

        if(!server->isListening()){
            qWarning()<<tr("Server: Cannot bind on port %1: %2").arg(port, server->errorString());
            this->quit();
            return;
        }
    }

    //!
    //! \brief onServerNewConnection
    //!
    void onServerNewConnection()
    {
        auto server=dynamic_cast<QLocalServer*>(QObject::sender());
        if(server==nullptr)
            return;

        auto socket = server->nextPendingConnection();
        QObject::connect(socket, &QLocalSocket::readyRead, this, &Server::onClientReadyRead);
        QObject::connect(socket, &QLocalSocket::disconnected, this, &Server::onClientDisconnected);
        if(socket!=nullptr){
            QByteArray identify;
            while(socket->waitForBytesWritten())
                identify+=socket->readAll();
            clientsidentifier[socket]=identify;
        }
    }

    //!
    //! \brief onClientReadyRead
    //!
    void onClientReadyRead()
    {
        auto socket = qobject_cast<QLocalSocket*>(QObject::sender());
        if(socket==nullptr)
            return;
        QByteArray bytes;
        while(socket->waitForBytesWritten())
            bytes+=socket->readAll();
        if(!bytes.isEmpty()){
            auto identifier=this->clientsidentifier.value(socket);
            emit dataReceived(identifier, bytes);
        }
    }

    //!
    //! \brief onClientDisconnected
    //!
    void onClientDisconnected()
    {
        auto socket = qobject_cast<QLocalSocket*>(QObject::sender());
        if (socket==nullptr)
            return;
        QObject::disconnect(socket, &QLocalSocket::readyRead, this, &Server::onClientReadyRead);
        QObject::disconnect(socket, &QLocalSocket::disconnected, this, &Server::onClientDisconnected);

        this->clientsidentifier.remove(socket);
        socket->close();
        socket->deleteLater();
    }

signals:
    void dataReceived(const QByteArray identify, const QByteArray bytes);

};


}


